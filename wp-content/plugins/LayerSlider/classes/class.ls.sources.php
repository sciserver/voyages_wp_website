...
class LS_Sources {
	...
	public static function addSkins($path) {
		...
		foreach($skinsPath as $key => $path) {
			...
			// Gather skin data
			$handle = mb_strtolower(basename($path));
			$skins[$handle] = array(
				'name' => $handle,
				'handle' => $handle,
				'dir' => $path,
	...
	public static function removeSkin($handle) {
		unset( self::$skins[ mb_strtolower($handle) ] );
	}
	...
	public static function getSkin($handle) {
		return self::$skins[ mb_strtolower($handle) ];
	}
	...
	public static function pathForSkin($handle) {
		return self::$skins[ mb_strtolower($handle) ]['dir'] . DIRECTORY_SEPARATOR;
	}
	...
	public static function urlForSkin($handle) {
		$path = self::$skins[ mb_strtolower($handle) ]['dir'];
	...
	public static function removeDemoSlider( $handle ) {
		unset( self::$sliders[ mb_strtolower($handle) ] );
	}
	...
	public static function getDemoSlider( $handle ) {
		return self::$sliders[ mb_strtolower($handle) ];
	}
	...
	public static function pathForDemoSlider( $handle ) {
		return self::$sliders[ mb_strtolower($handle) ]['dir'] . DIRECTORY_SEPARATOR;
	}
